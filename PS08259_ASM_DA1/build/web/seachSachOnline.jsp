...
<html>
    <head>
        ...
        <%if (!session.getAttribute("errorlg").equals("1")) {
            ...
            String ten = request.getParameter("txtSeach");
            String sql0 = "";
            List<Sach> sach;
            if (ma.equals("1")) {
                sql0 = " WHERE tensach like N'" + ten + "%'";
            } else if (ma.equals("2")) {
                sql0 = " WHERE matacgia like '" + ten + "%'";
            } else if (ma.equals("3")) {
                sql0 = " WHERE manxb like '" + ten + "%'";
            } else if (ma.equals("4")) {
                sql0 = " WHERE matl like '" + ten + "%'";
            }
            sach = SachDao.laydulieuSach(sql0);
            ...
            double dem = 0;
            for (Sach s : sach) {
                if (!s.getFiles().equals("")) {
                    dem += 1;
                }
            }
            double t = Math.ceil(dem / 3);
            for (int i = 3; i < t; i++) {
                height += 290;
            }
            out.println("<style>"
    ...
    <body>
        ...
        <nav> 
            ...
            <form action="seachSachOnline.jsp">
                <div class="seach1">
            ...
            <ul>
                <li><a>Thể loại</a><br/><div class="trong"></div>
                    <ul>
                        <%
                            for (Theloai tl : thl) {
                                out.println("<li><a href=\"seachSachOnline.jsp?txtSeach=" + tl.getMatl() + "&txtMa=4&action=seach\">" + tl.getTentl() + "</a></li></br>");
                            }%>
                    </ul><div class="trong"></div>
                    <a>Tác giả</a><br/><div class="trong"></div>
                    <ul>
                        <% for (Tacgia tg : tacgia1) {
                                out.println("<li><a href=\"seachSachOnline.jsp?txtSeach=" + tg.getMatacgia() + "&txtMa=2&action=seach\">" + tg.getTentacgia() + "</a></li><br/>");
                            }%>      
                    </ul>
                    <div class="trong"></div>
                    ...
                    <ul>
                        <%for (NhaXB n : nxb1) {
                                out.println("<li><a href=\"seachSachOnline.jsp?txtSeach=" + n.getManxb() + "&txtMa=3&action=seach\">" + n.getTennxb() + "</a></li>");
                            }%>   
                    </ul>
                </li>
            </ul>
            ...
            <%for (Sach s : sach) {
                    if (!s.getFiles().equals("")) {
                        ...
                        out.println("<div class=\"boxbook\">"
                                ...
                                + "</div>"
                                + "<a href=\"FILE\\" + name + "\"><button type =\"submit\" name=\"action\" value=\"file\">Click để đọc</button></a>"
                                + "</div>");
                    }
                }%>
        </div>
